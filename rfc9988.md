...
0. {:#c-nfc} Text strings in CRIs ("CRI text strings") are CBOR text
   ...
   the last two paragraphs of {{norm}}).
...
4. {:#c-ip-address} An IP address can be either an IPv4 address or an
   ...
   the discussion of zone-ids in {{cri-to-uri}}).
...
use of percent-encoded elements in the text-or-pet feature as per {{pet}}).
...
: If the CRI reference contains a `discard` item of value `true`, the
  ...
  the path contains a colon (":"), the path component is prefixed by "./" 
...
_Stand-in Items_ are allowed; otherwise, inconsistent interpretations at
...
This section discusses ways in which CRIs can be used directly in the context
...
`text-pet-sequence` subitem ({{pet}}) in this item fails this algorithm.
   ...
   4.  If the `host` component of »cri« is a `host-name`, include a
       ...
       the IP address, as derived in  the discussion of the representation
       of host-ip in{{cri-to-uri}} (note that zone-ids are never present in Uri-Host Options).

   6.  If the `port` subcomponent in a »cri« is not absent, then let »port« be that
       subcomponent's unsigned integer value; otherwise, let »port« be
       the default port number for the scheme.

   7.  If »port« does not equal the request's destination port,
       include a Uri-Port Option and let that option's value be »port«.

   8.  If the value of the `path` component of »cri« is empty or
   ...
   9.  If the value of the `query` component of »cri« is non-empty,
...
As a special exception, the expert may also make a registration for text
...
constrained applications. Such a registration follows the procedures for a third-party
...
1. {:#sp-leading-empty} Initial (Lone/Leading) Empty Path Segments:
  ...
  * *Leading empty path segments without authority*:
  ...
  the form `//foo`, the double slash would be mis-parsed as the first characters
  of an authority.
...
Resource Identifier (URI) Schemes" registry at the time the registry was set up with the new columns; see {{upd}}.
...
The authors of the current document completed this work with the goal of
achieving good integration with the potential use cases, both inside and outside of CoRAL.
